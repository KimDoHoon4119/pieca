...
<div style="float:left; margin:5% 0% 0% 10%;">
	...
	<input type="hidden" id="car_list_carno" value="${carList.size()}">
	<div id="car_list_out_container" style="float:left; width:40%; margin: 0% 0% 11% 0%;">
		...
		<c:if test="${item.no%2==1}">
			<div id="car_list_in_container${item.no}" onmouseover="zoom('${item.no}')"style="float:left; width:90%; cursor:pointer; border: 0px solid #747474; border-radius:6px; margin: 0% 0% 10% 10%; box-shadow: rgba(17, 17, 26, 0.1) 0px 8px 24px, rgba(17, 17, 26, 0.1) 0px 16px 56px, rgba(17, 17, 26, 0.1) 0px 24px 80px;">
				...
				<div id="car_list_fuel_sub" style="float:left; font-size:15px; height:45px; margin: 0% 0% 3% 0%;  color:#747474">
					<c:if test="${item.dt_max_fuel != 0}">
						<p>도심 ${item.dt_min_fuel / 10} ~ ${item.dt_max_fuel / 10}</p>
					</c:if>
					<c:if test="${item.dt_max_fuel == 0}">
						<p>도심 ${item.dt_min_fuel / 10}</p>
					</c:if>
					<c:if test="${item.high_max_fuel != 0}">
						<p>고속 ${item.high_min_fuel / 10} ~ ${item.high_max_fuel / 10}</p>
					</c:if>
					<c:if test="${item.high_max_fuel == 0}">
						<p>고속 ${item.high_min_fuel / 10}</p>
					</c:if>					
				...
				<div id="car_list_etc${item.no}" style="display: none;">
					...
					<div id="car_list_fuel_sub" style="float:left; font-size:15px; height:45px; margin: 0% 0% 0% 0%;  color:#747474">
						<c:if test="${item.max_output_motor != 0}">
					...
					<div id="car_list_torque_sub" style="float:left; font-size:15px; height:45px; margin: 0% 0% 0% 0%;  color:#747474">
						<c:if test="${item.max_torque_motor != 0}">
	...
	<div id="car_list_out_container" style="float:left; width:40%; margin: 0% 0% 13% 1%;">
		...
		<c:if test="${item.no%2==0}">
			<div id="car_list_in_container${item.no}" onmouseover="zoom('${item.no}')"style="float:left; width:90%; cursor:pointer; border: 0px solid #747474; border-radius:6px; margin: 0% 0% 10% 10%; box-shadow: rgba(17, 17, 26, 0.1) 0px 8px 24px, rgba(17, 17, 26, 0.1) 0px 16px 56px, rgba(17, 17, 26, 0.1) 0px 24px 80px;">
				...
				<div id ="car_list_description" onclick="show('${item.no}')">
...
<script>
carno_arr = new Array(45);
var carno_max = $("#car_list_carno").val()
$(document).ready(function(){
	runcnt = 0;
	var userid = $("#car_list_userid").val() //
	var carno_max = $("#car_list_carno").val() // 45
	for (var i = 1; i <= carno_max; i++ ){
		carno_arr[i] = likedec(i,userid)
		runcnt = i;
	}
})
	...
	if (userid == '') {
		...
		$.ajax({
		      ...
		      success:function(result){
		    	  ...
		    	  if(carno_arr[carno_max] != null){
			    	  likedec_1(result,carno)
		    	  }
		    	  /*if (result == false) {
		    		  $("#car_list_like_ok"+carno).hide();
		    		  $("#car_list_like_no"+carno).show();
...
function likedec_1(result,carno) {
	if (result == false) {
	  	$("#car_list_like_ok"+carno).hide();
  		$("#car_list_like_no"+carno).show();
	} else if (result == true){
