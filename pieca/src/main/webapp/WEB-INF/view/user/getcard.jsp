...
<body>
	...
	<div id="getcard_securitycode1_logo_box" style="position: relative; float:left; font-size:35px; margin:35px 0px 0px -50px;">
		<span id="getcard_securitycode1_logo" class="fa-sharp fa-solid fa-arrows-rotate"
			onclick="getRandomString();" style="color:#008000;"></span>
	</div>
	<%-- 보안코드 --%>
	...
	<input type="submit" id="getcard_submit" value="PIECA CARD 발급" style="width:79%; height:60px; border:2px solid #D5D5D5; border-radius: 6px; background-color: #D5D5D5;font-size: 22px; color:white; margin-top:10px; cursor:pointer;">
	<%-- 
...
$(document).ready(function(){
    ...
    if ($("#getcard_result").val() == 'ok') {
		let msg = "발급이 완료 되었습니다."
  		alert(msg)
        opener.document.location.reload();
        self.close();
    }
    
    if ($("#getcard_loginmode").val() == 'pieca') {
    	$("#chg_password").attr('type', 'password');
    } else if ($("#getcard_loginmode").val() == 'naver') {
        $("#getcard_password_input").val('Naver회원은 보안코드로 발급 가능');
    } else if ($("#getcard_loginmode").val() == 'kakao') {
        $("#getcard_password_input").val('kakao회원은 보안코드로 발급 가능');
    }
    
    if ($("#getcard_loginmode").val() != 'pieca') {
    	$("#getcard_password_mode").val("enable");
        $("#getcard_password_input").attr('type', 'text');
        $("#getcard_password_input").prop('readonly', true);
        $("#getcard_password_input").css({"background-color":"#F15F5F","border":"2px solid #F15F5F","font-size":"19px","color":"#FFFFFF"});
...
function submitChk() {
	   ...
	   (mode_security == 'enable')) {
	      ...
	      $("#getcard_submit").css("background-color","#008000");
	      $("#getcard_submit").css("border","2px solid #008000");
	   } else {
	      $("#getcard_submit").attr("disabled","disabled");
	      $("#getcard_submit").css("background-color","#D5D5D5");
	      $("#getcard_submit").css("border","2px solid #747474");
