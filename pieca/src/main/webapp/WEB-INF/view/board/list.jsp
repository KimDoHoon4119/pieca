...
<body>
	...
	<table class="w3-table-all w3-border">
		<tr>
			<form action="list" method="post" name="searchform">
				<td><input type="hidden" name="pageNum" value="1"> <input
					...
					name="searchtype" class="w3-select">
		...
		<c:if test="${listcount > 0}">
			...
			<c:forEach var="board" items="${boardlist}">
				<tr>
					...
					<td class="w3-left"><c:if test="${! empty board.fileurl}">
							<a href="file/${board.fileurl}">@</a>
						</c:if> <c:if test="${empty board.fileurl}">&nbsp;&nbsp;&nbsp;</c:if> <c:forEach
							begin="1" end="${board.grplevel}">&nbsp;&nbsp;</c:forEach> <c:if
							test="${board.grplevel > 0}">└</c:if> <%-- ㅂ 한자--%> <a
						href="detail?num=${board.num}">${board.title} <c:if
								test="${id==admin}">
		...
		<tr>
			<td colspan="5"><a href="write" class="w3-right">[글쓰기]</a></td>
		</tr>
	</table>
</body>
</html>