...
<div style="width:1920px; height:700px; display: flex; justify-content: center; align-items: center;">
	<table class="w3-table-all w3-border" style="width:70%; position: fixed; top: 100px; left: 275px; font-size:16px;">
		<tr>
		<form action="list" method="post" name="searchform">
			...
			<td colspan="2">
				<input type="text" name="searchcontent" value="${param.searchcontent }" class="w3-input" placeholder="검색어를 입력 하세요.">
			</td>
			<td>
					...
					<c:forEach items="${recog}" var="recog">
						<c:if test="${recog.num == board.num && board.boardid == 2}">
							...
							<c:if test="${recog.recog_Status==0}">
								<span id="up_check" style="color: black; font-size:20px;" class="fa-regular fa-circle-check"></span>
							</c:if>
		...
		<c:if test="${boardid == 1 && login.userid == 'admin'}">
			<tr>
				<td colspan="5">
					<a href="write" class="w3-left" style="color: #F15F5F">[글쓰기]</a>
				</td>
			</tr>
		</c:if>
		<c:if test="${boardid == 2}">
